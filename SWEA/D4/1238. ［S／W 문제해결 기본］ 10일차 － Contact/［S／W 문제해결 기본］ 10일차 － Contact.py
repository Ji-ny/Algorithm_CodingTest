...
def bfs(node):
    ...
    while (queue): # 큐에 값이 있을때까지
        # 현재 큐의 사이즈만큼 반복 (현재 너비만큼 )\
        nodeArray = []

        for _ in range(len(queue)):
            nowNode = queue.popleft()
            # 노드와 연결된 친구들 확인하기 
            for nextNode in array[nowNode]:
                # 꺼내고 -> 방문 확인하고 -> 넣고 
                # 다음 방문할 곳이 방문하지 않았다면?
                if not visited[nextNode]:
                    ...
                    # 이친구와 연결된 친구 연결 
                    queue.append(nextNode) # 노드 연결!
                    nodeArray.append(nextNode) # 결과 노드 연결!
        # print(nodeArray)
        if (len(nodeArray) > 0):
            lastArray = nodeArray
    return lastArray
...
for test_case in range(1, 11):
    ...
    visited = [False] * 101 # 1 ~  100 인원 방문 처리 
    # 방문 처리 배열 (배열로 -> i번째에 N번 노드 연결)

    array = [[] for _ in range(101)] # 연결 노드 관리 리스트

    # 입력 처리 
    nodeInput = list(map(int, input().split()))

    # from -> to 처리 
    for i in range(0, len(nodeInput), 2):
        # print(i)
        
        # 동일한 쌍이면 연결하지 않는다.
        if nodeInput[i+1] not in array[nodeInput[i]]:
            array[nodeInput[i]].append(nodeInput[i+1])

    queue = deque()

    print(f'#{test_case} {max(bfs(START))}')
